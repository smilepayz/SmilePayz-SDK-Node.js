const crypto = require('crypto');
const https = require('https');
const mySignature =require('../SignatureUtils')
const moment = require("moment/moment");
const { v4: uuidv4 } = require('uuid');


console.log('=====> step2 : Create Access Token');

async function payInRequest(sandboxMerchanteId,sandboxMerchanteCode,sandboxBaseDomain,minify){
    const privateKeyStr = "";
    const publicKeyStr = "";

    const referenceId = uuidv4();
    const timestamp = moment().format('YYYY-MM-DDTHH:mm:ssZ');
    const signData = referenceId +"|" + timestamp + '|' +  sandboxMerchanteCode + '|' + minify;
    const signature = mySignature.sha256Signature(signData,privateKeyStr)

    //options  you have changge hostname, timestamp,
    const options = {
        hostname: sandboxBaseDomain,
        port: 443,
        path: '/v2.0/transaction/pay-in',
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'REFERENCE-ID': referenceId,
            'X-TIMESTAMP': timestamp,
            'X-SIGNATURE': signature,
            'X-PARTNER-ID': sandboxMerchanteId
        }
    };

    //post request
    const req = https.request(options, (res) => {
        console.log(`Status Code: ${res.statusCode}`);

        res.on('data', (chunk) => {
            console.log(`Response Body: ${chunk}`);
        });
    });

    req.on('error', (error) => {
        console.error(`Error: ${error.message}`);
    });

    req.write(minify);
    req.end();
}
//get merchantId from merchant platform
const payInParam = {
    paymentMethod: 'PIX',
    payer: {
        name: 'payer name',
        phone: '12345678'
    },
    receiver:{
        name: 'payer name',
        phone: '12345678'
    },
    orderNo: 'T_1713176994197',
    purpose: 'test',
    productDetail: 'details',
    itemDetailList: [
        {
            name: "mac",
            quantity: 1,
            price: 100
        }
    ],
    billingAddress: {
        address: 'Ayu 1 No.B1 Pluit',
        city: 'jakarta',
        postalCode: '14450',
        phone: '018922990',
        countryCode: 'BRAZIL',
    },
    shippingAddress: {
        address: 'Ayu 1 No.B1 Pluit',
        city: 'jakarta',
        postalCode: '14450',
        phone: '018922990',
        countryCode: 'BRAZIL',
    },
    money:{
        currency: 'BRL',
        amount: 100,
    },
    merchant:{
        merchantId: 'sandbox-20019'
    },
    area: 13
}

const sandboxMerchanteCode = "6a58a603e5043290f4097ee4a7745661b3656932d4eebc3106b5dddc3af6e053";
const merchanteCode = "95b57c46b8c2e068982be23fb669a80612cad68e6ce6ba4f5af9ec20d23bb274";
const sandboxMerchanteId = "sandbox-20019";
const merchanteId = "20019";
const sandboxBaseDomain = "sandbox-gateway-test.smilepayz.com";
const baseUrl = "https://gateway-test.smilepayz.com";
const minify = JSON.stringify(payInParam);
payInRequest(sandboxMerchanteId,sandboxMerchanteCode,sandboxBaseDomain,minify);

//********** end post ***************
